...
class PlatformDetector {
  constructor () {
    ...
    if (undefined !== window) {
      if (window.navigator || navigator) {
        ...
        this._isOpera = null !== this.userAgent.match(/(?:^opera.+?version|opr)\/(\d+)/);

        var vendor = (this.nav && this.nav.vendor || '').toLowerCase();
        var match = /google inc/.test(vendor) ? this.userAgent.match(/(?:chrome|crios)\/(\d+)/) : null;
        ...
        this._isMobile = /android|webos|iphone|ipad|ipod|opera mini/i.test(this.userAgent) || this._iPadOS13;
        this._isIOS = /ipad|iphone|ipod/.test(this.userAgent) && !window.MSStream || this._iPadOS13;
      } else {
  ...
  logData () {
    this._logger.debug(`Firefox - ${this._isFirefox}, Chrome - ${this._isChrome}, Safari - ${this._isSafari}, IE - ${this._isIE}, Edge - ${this._isEdge}, Mobile - ${this._isMobile}, iOS - ${this._isIOS}, iPadOS - ${this._iPadOS13}`);
  }

  _isIOS13 (type) { // iPad, iPhone
    return (
      this.nav && this.nav.platform && (this.nav.platform.indexOf(type) !== -1 || (this.nav.platform === 'MacIntel' && this.nav.maxTouchPoints > 1 && !window.MSStream))
    );
  }
}
