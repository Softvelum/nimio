...
function initVUMeterSettings (settings, logger) {
  ...
  if (!["peak", "rms", "avg"].includes(settings.vuMeter.mode)) {
    logger.warn(`VU meter mode ${settings.vuMeter.mode} isn't recognized. Setting the "peak" mode by default`);
    settings.vuMeter.mode = "peak";
  }
  ...
  if (!['input', "output"].includes(settings.vuMeter.type)) {
    logger.warn(`VU meter type ${settings.vuMeter.type} isn't recognized. Setting the "output" type be default`);
    settings.vuMeter.type = "output";
  }
  ...
  if (!['AudioWorklet', "ScriptProcessor"].includes(settings.vuMeter.api)) {
    logger.warn(`VU meter api ${settings.vuMeter.api} isn't recognized. Setting "AudioWorklet" by default`);
    settings.vuMeter.api = "AudioWorklet";
  }
}
