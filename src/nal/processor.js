...
class NalProcessor {
  ...
  _process (pTime, frame) {
    ...
    while (curIdx < frame.byteLength - 4) {
      let nalSize = (frame[curIdx] << 24) | (frame[curIdx + 1] << 16) |
                    (frame[curIdx + 2] << 8) | frame[curIdx + 3];
      curIdx += 4;
      ...
      if (this._codec === 'H264') {
        ...
        if (nalu === AVC_NAL_UNIT_TYPE.SPS) {
          ...
          if (
            frame[sepStart] === 0 && frame[sepStart + 1] === 0 && frame[sepStart + 2] === 0 &&
            frame[sepStart + 3] === 1 && frame[sepStart + 4] === AVC_NAL_UNIT_TYPE.SEI
          ) {
            type = 'SEI';
            shift = 7;
          }
        ...
        if (nalu === HEVC_NAL_UNIT_TYPE.SPS) {
          type = 'SPS';
        } else if (nalu === HEVC_NAL_UNIT_TYPE.SEI_PREFIX ||
      ...
      if (type === 'SPS') {
        ...
        this._handlers[type].process(pTime, frame, start, curIdx + nalSize - 1, nalu);
      }
