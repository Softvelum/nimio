...
export class AbrController {
  ...
  start () {
    ...
    if( this.curStream && this.renditionProvider.getStreamsCount() > 1 ) {
      ...
      this._initTrials();
      this.phaseCount = 0;
      this.trials[this.curStream.orderedIdx].timer = setTimeout(this._trialComplete, 60000);
  ...
  playbackStalled (curBufLevel, lowBufferCount) {
    if( lowBufferCount > 0 ) {
      Logger.debug( `Low buffer count: ${lowBufferCount}, current buffer level: ${curBufLevel}` );
    }
  ...
  _setBufferingTime (bufferingTime) {
    ...
    this.stepDownBufferLevel = (this.bufferingTime > 700) ? 0.5 * this.bufferingTime / 1000 :
                                                            0.6 * this.bufferingTime / 1000;
    this.safeRunBufferLevel  = (this.bufferingTime > 700) ? 0.65 * this.bufferingTime / 1000 :
  ...
  _watchDog = (function () {
    if( this._evaluator.isRunning() ) {
      let curBufLevel = this._evaluator.calculateCurVideoStreamMetric( 'latestBufLevel' );
      if( curBufLevel < this.stepDownBufferLevel ) {
        Logger.debug('_watchDog interrupts abr ladder because current buffer level ' + curBufLevel + ' < ' + this.stepDownBufferLevel);
  ...
  _evalHandler = (function () {
    if( !this.isSwitchInProgressCallback() && !this.isSeekInProgressCallback() ) {
      let lowBufferCount = this._evaluator.calculateCurStreamMetric( 'latestLowBufferCount' );
      let curBufLevel = this._evaluator.calculateCurVideoStreamMetric( 'avg3secBufLevel' ); // 3 sec
      if( this.playbackStalled( curBufLevel, lowBufferCount ) ) {
        Logger.debug( 'evalHandler: playback stalled! Switch to lowest rendition.' );
        ...
        if( this.phaseCount >= 3 ) {
          let curBandwidth = this._evaluator.calculateCurStreamMetric( 'latestBandwidth' );
          let curRate = this._evaluator.calculateCurStreamMetric( 'latestRate' );
          curBandwidth += this._evaluator.calculateProbeStreamMetric( 'latestBandwidth' );

          Logger.debug( `eval: current buf: ${curBufLevel}, step down buf: ${this.stepDownBufferLevel}, current bw: ${curBandwidth}, current rate: ${curRate}` );
          if( curBufLevel < this.stepDownBufferLevel ) {
            this._evaluator.cancel();
            if( curRate > curBandwidth ) {
              let downIdx = this._evaluator.findRelevantStream( curBandwidth, curRate );
              if( downIdx !== this.curStream.orderedIdx ) {
                Logger.debug( `evalHandler: step down to ${this.renditionProvider.getRenditionName(downIdx)}` );
            ...
            let isSuitable = !this._isCurRenditionTop() && !this._evaluator.isRunning();
            if( isSuitable && (this.phaseCount >= this.maxPhases) && (curBandwidth > 0) &&
                (curBufLevel > this.safeRunBufferLevel) && (0 == lowBufferCount) ) {
              this._evaluator.run();
              this.phaseCount = 0;
            } else if (isSuitable) {
              Logger.debug(`ABR ladder isn't run: phaseCount = ${this.phaseCount}, curBandwidth = ${curBandwidth}, ` +
    ...
    if( this.trialsActive ) {
      for( let i = 0; i < renditions.length; i++ ) {
        if( this.trials[i] ) {
          ...
          if( (trialIdx === renditions[i].idx) && 
              (this.trials[i].stream === this.renditionProvider.getStream(trialIdx).stream) ) {
            success = true;
          }
        }
        if( !success ) break;
  ...
  _trialComplete = (function () {
    Logger.debug(`trial complete for ${this.curStream ? this.curStream.rendition : 'unknown'}`);
    if( this.curStream ) {
    ...
    if( orderedIdx !== this.curStream.orderedIdx ) {
      ...
      if( goUp ) {
        ...
        if( curTrial.timer ) {
          ...
          Logger.debug(`Increase trial for ${this.curStream.rendition + 'p'}, idx ${curTrial.idx} to ${curTrial.required}`);
        }
      }

      if( !goUp || (this.trials[orderedIdx].runs >= this.trials[orderedIdx].required) ) {
        this.switchRenditionCallback( this.renditionProvider.getRendition(orderedIdx).idx, mode );
      }
    }
  }
  ...
  _onLadderResult = (function ( streamIdx ) {
    Logger.debug( `ladder result: ${this.renditionProvider.getRenditionName(streamIdx)}, idx: ${streamIdx}, cur stream: ${this.curStream.rendition + 'p'}, idx: ${this.curStream.orderedIdx}`);
