...
function createTestBuffer(options = {}) {
    ...
    return SharedAudioBuffer.allocate(bufferSec, sampleRate, numChannels, sampleCount);
  }
  ...
  describe("SharedAudioBuffer", () => {
    ...
    it("handles concurrent-like read/write index contention", () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        expect(size).toBe((sab.getWriteIdx() + sab.capacity - sab.getReadIdx()) % sab.capacity);
      }
    });
