      ...
      if (!row.isEmpty()) {
        ...
        for (let c = 0; c < row.chars.length; ++c) {
          ...
          if (!prevPenState || !curPenState.equals(prevPenState)) {
            ...
            if (curPenState.underline) {
              curPenStateString += '_underline';
            }
            if (curPenState.italics) {
              curPenStateString += '_italics';
            }

            if (!this._styleStates[curPenStateString]) {
              this._styleStates[curPenStateString] = JSON.parse(JSON.stringify(curPenState));
            }
      ...
      for (let p = 0; p < regions[r].p.length; p++) {
        ...
        for (let s = 0; s < ptag.spans.length; s++) {
          ...
          if (span.line.length > 0) {
            ...
            spanElement.className = 'caption-span ' + span.name + ' customSpanColor';
