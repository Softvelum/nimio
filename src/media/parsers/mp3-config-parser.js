export function parseMp3Config (codecData) {
  ...
  if( (codecData[0] & 0xFF) == 0xFF && (codecData[1] & 0xE0) == 0xE0 ) {
    ...
    if( layerIndex != 0 ) {
      ...
      let bitrateIndex        = (codecData[2] >> 4) & 0x15;
      config.bitrate          = MPEG_BIT_RATE[ bitrateIndex ][ mpegVersionIndex ][ layerIndex ];
      let sampleRateIndex     = (codecData[2] >> 2) & 0x03;
      config.sampleRate       = MPEG_SAMPLING_RATES[ sampleRateIndex ][ mpegVersionIndex ];
