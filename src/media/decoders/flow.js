export class DecoderFlow {
  constructor(trackId, timescale, url) {
    ...
    this._decoder = new Worker(
      new URL(url, import.meta.url),
      { type: "module" },
    );
    this._decoder.addEventListener("message", (e) => {
      this._processDecoderMessage(e);
    });
  }
  ...
  processFrame( isSAP, data, timestamp, compositionOffset ) {
    ...
    if( this._transitStarted ) {
      if( this._transitParams.curStreamCancelled ) {
        ...
        this._logger.debug( `processFrame transition, current frame ts=${timestamp}, offset=${compositionOffset}, sap=${isSAP}` );
        ...
        if( this._needToCancelCurrentStream( timestamp, isSAP ) ) {
          this._logger.debug('processFrame cancel current rendition', this._transitParams.curStreamLastBufferedTs, isSAP );
  ...
  processTransitionFrame(isSAP, data, timestamp, compositionOffset) {
    ...
    if( tsbLen > 0 ) {
      if( undefined == this._transitParams.lastSampleDuration ) {
        this._transitParams.lastSampleDuration = (undefined !== _maxDuration) ? _maxDuration : 0;
      }
    ...
    this._pushTo( this._transitParams.startupBuffer, data, timestamp, compositionOffset, isSAP );
    if( this._needToSwitchToNewStream( timestamp, isSAP ) ) {
      ...
      if( this._transitParams.curStreamLastBufferedTs &&
      ...
      let i = 0;
      let minGap = Math.abs( this._transitParams.newSapTimes[0] - lastReceivedTs );
      for( i = this._transitParams.newSapTimes.length - 1; i >= 1; i-- ) {
        let curGap = Math.abs(this._transitParams.newSapTimes[i] - lastReceivedTs);
        if( curGap < minGap ) {
          minGap = curGap;
      ...
      if( i > 0 ) {
        ...
        if( iData ) {
          this._transitParams.composer.setTrackParams( this._transitParams.cTrackId, {codec: this._transitParams.codec,
                                                                            codecData: iData.data} );
          this._transitParams.initSegmentData = iData.data;
          this._transitParams.initSegment = this._transitParams.composer.initSegment();
      ...
      if( this._transitStarted ) {
        let edge = fCont[1];
        this._logger.debug( `transit buffer length = ${this._transitParams.startupBuffer.length}` );
        this._logger.debug( `===== Rendition switch GAP ===== ${1000 * edge} msec, allowance = ${1000 * fCont[2]}` );
        this.completeTransition();
        this.applyTransition();
        if (-edge > 10) {
          result.done = false;
          this._logger.error( `Correct rendition switch isn't possible, gap is too big (${edge}s). Reloading stream.` );
        }
      }
    }
    return result;
  ...
  _needToCancelCurrentStream(timestamp, isSAP) {
    ...
    if( tpLen > 0 ) {
      if( _sapAlignment ) {
        if( isSAP && ((timestamp >= this._transitParams.newSapTimes[0]) || (timestamp >= this._transitParams.newSapTimes[tpLen - 1])) ) {
          result = true;
        }
      } else {
        if( timestamp >= this._transitParams.newSapTimes[0] + 2 * (_transBuffering * _timescale / 1000) ) {
          this._logger.debug( 'Cancel current stream, because current timestamp is twice ahead possible buffer of new stream', timestamp, this._transitParams.newSapTimes[0] );
          result = true;
        } else {
          for( let i = this._transitParams.newSapTimes.length - 1; i >= 0; i-- ) {
            if( Math.abs(timestamp - this._transitParams.newSapTimes[i]) < _smoothBorder() ) {
              this._logger.debug( 'Cancel current stream. Timestamp ' + timestamp + ' is near new stream key frame ' + this._transitParams.newSapTimes[i] );
              result = true;
              break;
  ...
  _needToSwitchToNewStream(timestamp) {
    ...
    if( _sapAlignment && (TRANSITION_MODE.ABRUPT != this._transitParams.mode) ) {
      if( tsDiff >= maxShift ) {
        if( curSapTimesLength > 0 ) {
          ...
          for( let j = 0; j < curSapTimesLength; j++ ) {
            if( (this._transitParams.curSapTimes[j] < this._transitParams.newSapTimes[curK]) ||
                (this._transitParams.curSapTimes[j] <= _lastProcessedTimestamp)) {
              continue;
            }
            for( let k = curK; k < newSapTimesLength; k++ ) {
              curK = k;
              if( this._transitParams.newSapTimes[k] > this._transitParams.curSapTimes[j] ) {
                break;
              } else if( this._transitParams.newSapTimes[k] == this._transitParams.curSapTimes[j] ) {
                this._logger.debug( 'Switch to new stream because of SAP alignment' );
                result = true;
              }
            }
            if( result ) break;
      ...
      if( !result && (curSapTimesLength >= 1) && (newSapTimesLength >= 2) &&
          ((this._transitParams.newSapTimes[ newSapTimesLength - 1] > this._transitParams.curSapTimes[0]) ||
           (this._transitParams.newSapTimes[ newSapTimesLength - 1] >= this._transitParams.curSapTimes[curSapTimesLength - 1])) ) {
        this._logger.debug( 'Switch to new stream SAP alignment does not work!' );
        result = true;
      }
    } else if( (tsDiff >= 2 * maxShift) &&
               (timestamp >= this._transitParams.curStreamLastBufferedTs || undefined == this._transitParams.curStreamLastBufferedTs) ) {
      this._logger.debug( 'Switch to new stream because transition buffer is twice filled' );
      result = true;
    } else if( tsDiff >= maxShift ) {
      if( this._transitParams.curStreamCancelled &&
          (timestamp >= this._transitParams.curStreamLastBufferedTs || undefined == this._transitParams.curStreamLastBufferedTs) ) {
        this._logger.debug( 'Switch to new stream because buffer is filled and current stream is cancelled' );
        result = true;
      } else {
        for( let i = this._transitParams.newSapTimes.length - 1; i >= 0; i-- ) {
          if( Math.abs(this._transitParams.curStreamLastBufferedTs - this._transitParams.newSapTimes[i]) < _smoothBorder() ) {
            this._logger.debug( 'Switch to new stream, because new key frame ' + this._transitParams.newSapTimes[i] + ' is near to current latest timestamp ' + this._transitParams.curStreamLastBufferedTs );
            result = true;
            break;
          } else if( this._transitParams.curStreamLastBufferedTs > this._transitParams.newSapTimes[i] ) {
            break;
          }
        }
        if( !result && (timestamp + 2 * maxShift < this._transitParams.curStreamLastBufferedTs) ) {
          if( tsDiff >= 10 * _timescale ) {
            this._logger.error( 'Error: new stream is ' + ((this._transitParams.curStreamLastBufferedTs - timestamp) / _timescale) + ' seconds behind previous stream. Halting.' );
            result = true;
          }
        }
      }
