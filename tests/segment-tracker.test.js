...
describe('PlaybackSegmentTracker', () => {
  ...
  it('setup sorts segments if not sorted', () => {
    const tracker = PlaybackSegmentTracker.getInstance(instanceId + '-unsorted');
  ...
  it('get returns undefined if not set up', () => {
    const tracker = PlaybackSegmentTracker.getInstance(instanceId + '-no-setup');
  ...
  it('get respects segment boundaries (inclusive start, exclusive end)', () => {
    const tracker = PlaybackSegmentTracker.getInstance(instanceId + '-boundary');

    tracker.setup(
      createSegments([{ start: 0, duration: 10 }])
    );
