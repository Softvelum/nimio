...
class PlaybackProgressService {
  ...
  updateVodProgress (pos, dur) {
    ...
    if (!isNaN(dur) && dur !== this._vodDuration) {
      this._logger.debug(`updateVodProgress prev duration = ${this._vodDuration}, new duration = ${dur}`);
  ...
  updateLiveProgress (pos, dur) {
    ...
    if (this._extProgressNotifier) {
      this._extProgressNotifier.liveProgress(this._livePosition, this._liveDuration);
    }
  }
  ...
  _doPositionUpdate (position) {
    ...
    if (position >= this._fullVodDuration()) {
      if (this._positionChangeCallback('live', this._totalDuration() - position)) {
  ...
  _calcPosition (type) {
    ...
    if (position > 1) {
      this._logger.error('position is greater than the whole scale', position, type);
      position = 1;
    }
...
PlaybackProgressService.prototype.setExtProgressNotifier = function (inst) {
  this._extProgressNotifier = inst;
};

PlaybackProgressService.prototype.updateVodPosition = function (position) {
...
PlaybackProgressService.prototype.updateLivePosition = function (buffer) {
