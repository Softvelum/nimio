...
export class Cea608Processor {
  constructor (instName, captionPresenter) {
    let cueHandlers = [];
    for (let i = 0; i < 4; i++) {
      cueHandlers[i] = new CueHandler(captionPresenter, `CC${i + 1}`);
    }
    this.type = 'cea608';

    this._parser1 = new Cea608Parser(instName, 1, cueHandlers[0], cueHandlers[1]);
    this._parser2 = new Cea608Parser(instName, 2, cueHandlers[2], cueHandlers[3]);
  }
  ...
  isMatching (payloadType, payloadSize, frame, pos) {
    ...
    let userIdentifier = frame[pos + 3] << 24 | frame[pos + 4] << 16 | frame[pos + 5] << 8 | frame[pos + 6];
